...
export async function registerRoutes(httpServer: Server, app: Express) {
  ...
  app.post("/api/auth/login", (req, res) => {
    ...
    const profile = user.role === "freelancer" ? storage.getProfileByUserId(user.id) : null;
  ...
  app.get("/api/profiles", (req, res) => {
    ...
    const profiles = storage.getProfiles({ specialism, availability, search });
    res.json(profiles);
  });

  app.get("/api/profiles/featured", (req, res) => {
    res.json(storage.getFeaturedProfiles());
  });

  app.get("/api/profiles/:id", (req, res) => {
    const pw = storage.getProfile(Number(req.params.id));
    if (!pw) return res.status(404).json({ error: "Profile not found" });
    const reviews = storage.getReviewsByProfile(pw.profile.id);
    res.json({ ...pw, reviews });
  });

  app.patch("/api/profiles/:id", (req, res) => {
    const updated = storage.updateProfile(Number(req.params.id), req.body);
  ...
  app.get("/api/messages/:userId/conversations", (req, res) => {
    const convs = storage.getConversations(Number(req.params.userId));
    res.json(convs);
  });

  app.get("/api/messages/:fromId/:toId", (req, res) => {
    const msgs = storage.getMessagesBetween(Number(req.params.fromId), Number(req.params.toId));
    storage.markMessagesRead(Number(req.params.fromId), Number(req.params.toId));
    res.json(msgs);
  });
  ...
  // ─── Saved ────────────────────────────────────────────────────────────────
  app.get("/api/saved/:clientId", (req, res) => {
    res.json(storage.getSaved(Number(req.params.clientId)));
  });

  app.post("/api/saved/toggle", (req, res) => {
    const { clientId, profileId } = req.body;
    const saved = storage.toggleSaved(Number(clientId), Number(profileId));
    res.json({ saved });
  });

  app.get("/api/saved/:clientId/:profileId", (req, res) => {
    const saved = storage.isSaved(Number(req.params.clientId), Number(req.params.profileId));
    res.json({ saved });
  });
  ...
  app.post("/api/ai-search", (req, res) => {
    ...
    const results = profiles.length > 0 ? profiles : storage.getProfiles({});
  ...
  app.get("/api/feed", (req, res) => {
    ...
    res.json(storage.getFeedPosts(limit, offset, viewerUserId));
  });
  ...
  app.post("/api/feed", (req, res) => {
    try {
      ...
      const pw = storage.getPost(post.id);
      res.json(pw);
    } catch (e: any) {
      res.status(400).json({ error: e.message });
    }
  ...
  app.delete("/api/feed/:id", (req, res) => {
    ...
    const ok = storage.deletePost(Number(req.params.id), Number(userId));
  ...
  app.post("/api/feed/:id/like", (req, res) => {
    ...
    const liked = storage.toggleLike(Number(req.params.id), Number(userId));
  ...
  app.post("/api/feed/:id/comments", (req, res) => {
    try {
      ...
      const comment = storage.createComment(data);
      res.json(comment);
    } catch (e: any) {
      res.status(400).json({ error: e.message });
    }
  ...
  app.post("/api/pro/subscribe", (req, res) => {
    const { userId } = req.body;
    if (!userId) return res.status(400).json({ error: "userId required" });
    const profile = storage.getProfileByUserId(Number(userId));
    if (!profile) return res.status(404).json({ error: "Profile not found. Only freelancers can subscribe." });
    const updated = storage.subscribePro(profile.id);
    res.json({ success: true, profile: updated });
  });

  app.get("/api/pro/status/:userId", (req, res) => {
    const profile = storage.getProfileByUserId(Number(req.params.userId));
  ...
  app.get("/api/projects", (req, res) => {
    ...
    if (!userId) return res.status(400).json({ error: "userId required" });
    res.json(storage.getProjectsForUser(Number(userId)));
  });

  app.get("/api/projects/:id", (req, res) => {
    const pw = storage.getProject(Number(req.params.id));
  ...
  app.post("/api/projects", (req, res) => {
    try {
      ...
      const full = storage.getProject(project.id);
      res.json(full);
    } catch (e: any) {
      res.status(400).json({ error: e.message });
    }
  ...
  app.post("/api/projects/:id/advance", (req, res) => {
    ...
    const updated = storage.advanceProjectStage(Number(req.params.id), note || "", Number(authorId));
    if (!updated) return res.status(404).json({ error: "Project not found" });
    res.json(storage.getProject(updated.id));
  });
  ...
  app.post("/api/projects/:id/updates", (req, res) => {
    try {
      ...
      const update = storage.addProjectUpdate(data);
      res.json(update);
    } catch (e: any) {
      res.status(400).json({ error: e.message });
    }
  ...
  app.get("/api/projects/:id/updates", (req, res) => {
    res.json(storage.getProjectUpdates(Number(req.params.id)));
  });

  // ─── Briefs ────────────────────────────────────────────────────────────────
  app.get("/api/briefs", (req, res) => {
    const { category, location } = req.query;
    let briefs = storage.getBriefs();
  ...
  app.get("/api/briefs/:id", (req, res) => {
    const brief = storage.getBrief(Number(req.params.id));
  ...
  app.post("/api/briefs", (req, res) => {
    try {
      ...
      const brief = storage.createBrief(data);
      res.json(brief);
    } catch (e: any) {
      res.status(400).json({ error: e.message });
    }
