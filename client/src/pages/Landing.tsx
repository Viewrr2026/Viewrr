...
export default function Landing() {
  ...
  useEffect(() => {
    // Mount the video element only after the page has fully painted.
    // Safari renders its play-button overlay the moment a <video> tag enters
    // the DOM — delaying the mount means the poster background is already
    // painted and visible before Safari ever gets a video element to overlay.
    const mountTimer = setTimeout(() => setVideoMounted(true), 800);
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <section className="relative overflow-hidden">
        {/* Background video — poster via CSS backgroundImage so it's instant-on */}
        <div
          className="absolute inset-0 z-0"
          style={{
            overflow: "hidden",
            userSelect: "none",
            // This CSS background is painted synchronously by the browser before
            // any JS runs — it's the true poster and is never absent.
            backgroundImage: "url('/videos/hero_showreel_poster.jpg')",
            backgroundSize: "cover",
            backgroundPosition: "center",
          }}
        >
          {/* Video — opacity:0 until playing so the play button is never visible */}
          {videoMounted && (
            <video
              ref={videoRef}
              ...
              style={{
                ...
                transition: videoVisible ? "opacity 0.8s ease" : "none",
              } as React.CSSProperties}
              onContextMenu={e => e.preventDefault()}
            />
          )}
          {/* Full-cover poster image at z-index 1 — sits above the video and fades
              out once playing. This guarantees the play button (which Safari paints
              inside the video's compositing layer) is always covered, including the
              right side of the hero where the gradient is semi-transparent. */}
          <img
          ...
          />
          {/* Absorbs pointer events above the poster so no browser chrome shows */}
          <div
            aria-hidden="true"
            style={{
