...
function ExpressInterestModal({ brief, open, onClose }: { brief: Brief | null; open: boolean; onClose: () => void }) {
    ...
    try {
      const res = await fetch("/api/interests", {
        ...
        body: JSON.stringify({
          ...
          proposedPricePence: proposedPrice ? Math.round(parseFloat(proposedPrice) * 100) : null,
  ...
  function handleClose() {
    ...
    setProposedPrice(""); setPriceBreakdown(""); setSubmitting(false);
    onClose();
  }
          ...
          <>
            ...
            <form onSubmit={submit} className="space-y-4">
              ...
              <div className="space-y-1.5">
                ...
                <p className="text-xs text-muted-foreground">Fixed price for the entire project. The client will see this before accepting.</p>
              </div>

              {/* Price breakdown */}
              <div className="space-y-1.5">
                <label className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Price breakdown <span className="text-muted-foreground/60 font-normal normal-case">(optional)</span></label>
                <Textarea
                  placeholder={`e.g.\n• Discovery & planning: £200\n• Design & build: £600\n• Revisions: £200`}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1.5">
                  <label className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Day rate (info only)</label>
              ...
              <Button type="submit" disabled={!coverNote.trim() || !proposedPrice || submitting} className="w-full bg-primary hover:bg-primary/90 text-white rounded-full">
