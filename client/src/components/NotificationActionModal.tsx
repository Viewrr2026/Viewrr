...
const SPEC_ICONS: Record<string, typeof Video> = {
  "Videographer": Video,
...
function ConnectionRequestPanel({
  ...
  // Fetch the sender's full profile
  ...
  // Fetch connection status to get the requestId
  const { data: connStatus } = useQuery<any>({
    queryKey: ["/api/connections/status", user?.id, notification?.actorId],
    queryFn: async () => {
      if (!user) return null;
      const res = await apiRequest("GET", `/api/connections/status?userA=${user.id}&userB=${notification!.actorId}`);
      return res.json();
    },
    enabled: !!user && !!notification?.actorId,
  });

  async function respond(status: "accepted" | "declined") {
    if (!user || !connStatus?.requestId) return;
    setResponding(true);
    try {
      await apiRequest("POST", "/api/connections/respond", {
        requestId: connStatus.requestId,
        responderId: user.id,
        senderId: notification!.actorId,
        status,
      });
      queryClient.invalidateQueries({ queryKey: ["/api/connections"] });
      queryClient.invalidateQueries({ queryKey: ["/api/connections/status", user.id, notification!.actorId] });
      setDone(status);
    } catch {
      // silent — toast not available here but failure is rare
    } finally {
      setResponding(false);
    }
  }

  const pw = profileData;
  const prof = pw?.profile;
  const senderUser = pw?.user;
  const specialisms: string[] = JSON.parse(prof?.specialisms || "[]");
  const skills: string[] = JSON.parse(prof?.skills || "[]");
  const badges: string[] = JSON.parse(prof?.badges || "[]");
  const portfolioItems: any[] = JSON.parse(prof?.portfolioItems || "[]");
  const specialism = specialisms[0] || "Creative";
  const Icon = SPEC_ICONS[specialism] || Video;
  const alreadyConnected = connStatus?.status === "accepted";
  ...
  return (
    <ModalShell onClose={onClose}>
      ...
      <div className="flex items-center gap-3 px-4 py-3 border-b border-border">
        ...
        <div>
          <p className="text-sm font-semibold">Connection Request</p>
          <p className="text-xs text-muted-foreground">{notification?.actorName} wants to connect</p>
        </div>
      </div>
      ...
      <div className="overflow-y-auto flex-1">
        {profileLoading && (
          <div className="p-4 space-y-3">
            <div className="h-36 bg-secondary rounded-xl animate-pulse" />
        ...
        {!profileLoading && pw && (
          <div className="p-4 space-y-0">
            {/* Mini card — mirrors FreelancerCard layout */}
            <div className="rounded-2xl border border-border overflow-hidden bg-card">
              ...
              <div className="relative h-40 overflow-hidden bg-muted">
                ...
                {/* Specialism pill */}
                <div className="absolute top-2.5 left-2.5 flex items-center gap-1.5 bg-background/90 backdrop-blur-sm rounded-full px-2.5 py-1 text-xs font-semibold">
                  <Icon size={10} className="text-primary" />
                  {specialism}
                </div>
                {/* Pro badge */}
                {prof?.isPro === 1 && (
                  <div className="absolute bottom-2.5 left-2.5 flex items-center gap-1 rounded-full px-2 py-0.5 text-xs font-bold"
              ...
              <div className="p-3.5">
                <div className="flex items-start gap-2.5 mb-2.5">
                  ...
                  <div className="min-w-0 flex-1">
                    <h3 className="font-semibold text-sm truncate">{senderUser?.name || notification?.actorName}</h3>
                    {senderUser?.headline && (
                      <p className="text-xs text-muted-foreground truncate">{senderUser.headline}</p>
                    )}
                    {senderUser?.location && (
                      <p className="flex items-center gap-1 text-xs text-muted-foreground mt-0.5">
                        <MapPin size={9} /> {senderUser.location}
                      </p>
                    )}
                  </div>
                </div>
                ...
                {(prof?.rating || 0) > 0 && (
                  <div className="flex items-center gap-1 mb-2">
                    ...
                    <span className="text-xs font-semibold">{(prof.rating || 0).toFixed(1)}</span>
                    <span className="text-xs text-muted-foreground">({prof.reviewCount || 0})</span>
                    <span className="ml-auto text-xs text-muted-foreground">{prof.projectCount || 0} projects</span>
                  </div>
                )}
                ...
                {prof?.dayRate && (
                  <div className="pt-2.5 border-t border-border">
                    <span className="font-bold text-sm">£{prof.dayRate}<span className="text-xs font-normal text-muted-foreground">/day</span></span>
                  </div>
                )}
              </div>
            </div>
        ...
        {!profileLoading && !pw && (
          <div className="flex flex-col items-center py-10 gap-2 text-muted-foreground text-sm">
            ...
            <p>{notification?.actorName} sent you a connection request</p>
          </div>
        )}
      </div>
      ...
      <div className="px-4 py-3 border-t border-border">
        {done === "accepted" || alreadyConnected ? (
          <div className="flex items-center gap-2 rounded-xl px-4 py-3 text-sm font-semibold bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400">
            <UserCheck size={15} /> You are now connected with {notification?.actorName}
          </div>
        ) : done === "declined" ? (
          <div className="flex items-center gap-2 rounded-xl px-4 py-3 text-sm font-semibold bg-muted text-muted-foreground">
            <XCircle size={15} /> Request declined
          </div>
        ) : (
          <div className="flex gap-2">
            <Button
              className="flex-1 bg-primary hover:bg-primary/90 text-white rounded-full text-sm"
              onClick={() => respond("accepted")}
              disabled={responding || !connStatus?.requestId}
            >
              <UserCheck size={14} className="mr-1.5" /> Accept
            </Button>
            <Button
              variant="outline"
              className="flex-1 rounded-full text-sm text-muted-foreground hover:text-destructive hover:border-destructive"
...
export default function NotificationActionModal({ open, onClose, notification }: NotificationModalProps) {
  ...
  if (type === "connection_request") {
    return <ConnectionRequestPanel notification={notification} onClose={onClose} />;
  }
    ...
    return (
      <ModalShell onClose={onClose}>
        <div className="flex flex-col items-center text-center px-6 py-10 gap-4">
          ...
          <p className="text-sm text-muted-foreground bg-secondary/60 rounded-xl px-4 py-3">
            You can now message each other directly from your Connections panel in the Dashboard.
          </p>
          <Button className="bg-primary hover:bg-primary/90 text-white rounded-full w-full" onClick={onClose}>
            Got it
          </Button>
  ...
  return (
    <ModalShell onClose={onClose}>
      <div className="flex flex-col items-center justify-center py-12 gap-3 px-6 text-center">
        ...
        <Button className="bg-primary hover:bg-primary/90 text-white rounded-full" onClick={onClose}>Got it</Button>
      </div>
    </ModalShell>
  );
}
