...
function StatusBadge({ status }: { status: string }) {
  const map: Record<string, { label: string; icon: React.ReactNode; cls: string }> = {
    ...
    counter_offered: { label: "Counter offer",  icon: <ArrowLeftRight size={11} />, cls: "bg-violet-100 text-violet-700 dark:bg-violet-900/30 dark:text-violet-400" },
  };
...
function InterestDetail({
  ...
  // Accept-counter mutation (freelancer side)
  const acceptCounterMutation = useMutation({
    mutationFn: async () => {
      const res = await fetch(`/api/interests/${interest.id}/accept-counter`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
      });
      if (!res.ok) throw new Error("Accept counter failed");
      return res.json();
    },
    onSuccess: () => {
  ...
  return (
    <div className="flex flex-col h-full min-h-[560px]">
      ...
      <div className="mx-4 mb-4 bg-muted/40 border border-border rounded-xl p-4 space-y-3">
        ...
        {/* Proposed / counter price info */}
        {((interest as any).proposedPricePence || (interest as any).counterOfferPence) && (
          <div className="rounded-xl border border-primary/20 bg-primary/4 overflow-hidden">
              ...
              <div className="flex items-center justify-between px-3 py-2">
                <div className="flex items-center gap-1.5">
                  ...
                  <p className="text-[10px] font-semibold uppercase tracking-wide text-muted-foreground">
                    {isFreelancer ? "Your proposed price" : "Proposed price"}
                  </p>
                </div>
            ...
            {(interest as any).counterOfferPence && (
              <div className="flex items-center justify-between px-3 py-2 border-t border-primary/15">
                <div className="flex items-center gap-1.5">
                  <ArrowLeftRight size={11} className="text-violet-500" />
                  <p className="text-[10px] font-semibold uppercase tracking-wide text-violet-600 dark:text-violet-400">
                    {isFreelancer ? "Client counter-offer" : "Your counter-offer"}
                  </p>
                </div>
            ...
            {/* Breakdown */}
            {(interest as any).priceBreakdown && (
              <div className="px-3 py-2 border-t border-primary/15">
                <p className="text-[10px] font-semibold uppercase tracking-wide text-muted-foreground mb-1">Breakdown</p>
        ...
        {/* Freelancer: accept the counter-offer */}
        {isFreelancer && interest.status === "counter_offered" && (interest as any).counterOfferPence && (
          <div className="rounded-xl border border-violet-200 dark:border-violet-800 bg-violet-50 dark:bg-violet-950/30 p-3 space-y-2">
            <p className="text-xs font-semibold text-violet-700 dark:text-violet-300">
              The client has countered at {formatGBP((interest as any).counterOfferPence)}.
            </p>
            <p className="text-[11px] text-muted-foreground">Accept their counter-offer to kick off the project at this agreed price.</p>
            <Button
              size="sm"
              className="bg-violet-600 hover:bg-violet-700 text-white rounded-full text-xs px-4 gap-1"
              disabled={acceptCounterMutation.isPending}
              onClick={() => acceptCounterMutation.mutate()}
            >
              <CheckCircle2 size={11} /> {acceptCounterMutation.isPending ? "Accepting…" : "Accept counter-offer"}
            </Button>
          </div>
        )}
